        ...
        for epoch in range(cfg['epochs']):
            ...
            for images, labels, batch_image_paths in tqdm(val_loader):
                ...
                with autocast("cuda"), torch.no_grad():
                    outputs = torch.sigmoid(model(images)).data.cpu().numpy()
                    ema_outputs = torch.sigmoid(model_ema.module(images)).data.cpu().numpy()
                
                for image_path, pred, ema_pred in zip(batch_image_paths, outputs, ema_outputs):
            ...
            for SeriesInstanceUID in tqdm(np.unique(val_df.SeriesInstanceUID.values)):
                ...
                for _, row in grp.iterrows():
                    ser_pred.append(pred_dict[row['image_path']])
            ...
            val_auc = get_weighted_auc(gts, preds)
            ema_val_auc = get_weighted_auc(gts, ema_preds)
            
            print('Fold: {} | Epoch {} | train loss: {:.5f} | val_auc: {:.5f} | ema_val_auc: {:.5f}'.format(fold, epoch, train_loss, val_auc, ema_val_auc))
            logger.append([epoch, round(optimizer.param_groups[0]['lr'], 8), round(train_loss, 5), round(val_auc, 5), round(ema_val_auc, 5)])
            log_df = pd.DataFrame(data=np.array(logger), columns=['epoch', 'lr', 'train_loss', 'val_auc','ema_val_auc'])
            log_df.to_csv(LOG, index=False)

            if val_auc_max < max(val_auc, ema_val_auc):
                print('val auc improved from {:.5f} to {:.5f} saving model to {}'.format(val_auc_max, max(val_auc, ema_val_auc), CHECKPOINT))
                val_auc_max = max(val_auc, ema_val_auc)
                if val_auc > ema_val_auc:
