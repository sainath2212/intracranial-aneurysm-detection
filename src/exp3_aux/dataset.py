...
class AneurysmDataset(Dataset):
    def __init__(self, df, brain_box_df, image_size, mode, crop_ratio=1):
        ...
        if self.mode == 'train':
            ...
            self.df = pd.concat([neg_df, pos_df], ignore_index=True)
        ...
        if self.mode == 'train':
            ...
            if row['has_box'] == 1:
                ...
                for ann in annotations:
                    ...
                    if clsname == 'aneurysm':
                        mask[y1:y2,x1:x2,0] = 255*np.ones((y2-y1, x2-x1), dtype=np.uint8)
                    elif clsname == 'aneurysm_mri_t2':
                        mask[y1:y2,x1:x2,1] = 255*np.ones((y2-y1, x2-x1), dtype=np.uint8)
                    else:
                        raise ValueError()
            mask = mask[brain_y1:brain_y2, brain_x1:brain_x2, :]
...
class AneurysmTestDataset(Dataset):
    def __init__(self, df, brain_box_df, image_size):
        self.df = df.reset_index(drop=True)
        self.image_size = image_size
